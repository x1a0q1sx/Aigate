...
每次 Auto、Combo 与直连请求都会生成一条不含提示词和密钥的决策快照，保存 30 天：

- Auto 候选排名，以及速度 / 智力 / 稳定性 / 综合分拆分。
- 最终选中模型、选中原因、被跳过候选及原因。
- 完整上游尝试链、每次尝试耗时、首字延迟、错误和 fallback 次数。
- 最近 1 小时至 30 天的成功率、回退率与平均决策耗时。
...
1. 使用 SQLite 在线备份创建恢复点，同时保存 `config.yaml`、当前前端产物和 SHA-256 清单。
2. 拉取代码并按需安装依赖、原子构建前端。
3. 在数据库副本上执行迁移，运行 Python 编译、完整 pytest 和 FastAPI 导入检查。
4. 通过 PM2 重启，并验证 HTTP 健康状态。
5. 任一步失败时自动恢复旧提交、数据库、配置与前端，再拉起旧版本。
...
恢复点保存在 `data/backups/update-*`，自动保留最近 5 份；设置页可查看阶段、恢复点和回滚结果。
...
| 路由决策 | 候选评分拆分、选中/跳过原因、上游尝试与 fallback 链路 |
