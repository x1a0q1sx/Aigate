...
def clean():
    ...
    # 裁剪数据库：保留 3 个内置 Free Tier 服务商及其模型，清空其余用户数据
    ...
    if errors:
        ...
        print(f"✅ 全部清理完成，共 {deleted} 项。3 个内置 Free Tier 服务商已保留，config 密钥已擦除。")
...
def ensure_builtin_providers():
    ...
    KEEP_NAMES = {"MiMo Code Free", "AtomCode", "MiMo", "OpenCode"}
    ...
    try:
        asyncio.run(_run())
        print("   ✓ 已保留 3 个内置 Free Tier 服务商（MiMo/OpenCode/AtomCode）及其模型，其余用户数据已清空")
    except Exception as e:
        print(f"   ✗ 保留内置服务商失败: {e}")
