    ...
    <div class="card">
      <table>
        <thead>
          <tr>
            <th>模型 ID</th>
            <th class="sortable" @click="toggleSort('display_name')">显示名 <span class="sort-ind" v-if="sortKey==='display_name'">{{ sortOrder==='asc'?'▲':'▼' }}</span></th>
            <th class="provider-col">服务商</th>
            <th>价格 (输入/输出)</th>
            <th class="sortable" @click="toggleSort('success_rate')">成功率 <span class="sort-ind" v-if="sortKey==='success_rate'">{{ sortOrder==='asc'?'▲':'▼' }}</span></th>
            <th>免费</th>
            <th>Auto</th>
            <th>延迟/TPS</th>
            <th>干预</th>
        ...
        <tbody>
          ...
          <tr v-for="m in sortedModels" :key="m.id" :style="{opacity: m.enabled ? 1 : 0.5}">
    ...
    sortedModels() {
      ...
      arr.sort((a, b) => {
        ...
        if (key === 'display_name') {
          ...
          // null 永远排到最后，无论升/降序
          if (a.success_rate === null || a.success_rate === undefined) return 1
          if (b.success_rate === null || b.success_rate === undefined) return -1
...
.sort-ind { color: var(--primary); font-size: 11px; }
.modal-overlay {
  position: fixed;
  top: 0; left: 0; right: 0; bottom: 0;
