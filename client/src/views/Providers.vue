...
export default {
  ...
  methods: {
    ...
    async refreshAllModels() { this.busy.refreshModels = true; try { const r = await api.refreshModels(); await this.load(); alert(`刷新完成：新增 ${r.added}，更新 ${r.updated}，总计 ${r.total}`) } catch (e) { alert('刷新失败: ' + e.message) } finally { this.busy.refreshModels = false } },
    async refreshProviderModels(p) { this.busy.refreshModels = true; try { const r = await api.refreshModels(p.id); await this.load(); alert(`刷新完成：新增 ${r.added}，更新 ${r.updated}`) } catch (e) { alert('刷新失败: ' + e.message) } finally { this.busy.refreshModels = false } },
