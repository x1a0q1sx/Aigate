  ...
  <div class="media-page">
    ...
    <div v-show="tab==='image'" class="panel">
      <div class="grid">
        <div class="field">
          <label>服务商</label>
          <select v-model="imgForm.provider_id">
        ...
        <div class="field">
          <label>模型</label>
          <input v-model="imgForm.model" placeholder="dall-e-3 / flux.1-schnell / agnes-image-2.1-flash" />
        </div>
      ...
      <button class="btn-gen" :disabled="imgGenerating || !imgForm.prompt || !imgForm.provider_id" @click="generateImage">
    ...
    <div v-show="tab==='video'" class="panel">
      <div class="grid">
        <div class="field">
          <label>服务商</label>
          <select v-model="vidForm.provider_id">
            <option :value="null" disabled>选择服务商</option>
            <option v-for="p in providers" :key="p.id" :value="p.id">{{ p.name }}</option>
          </select>
        </div>
        <div class="field">
          <label>模型</label>
          <input v-model="vidForm.model" placeholder="如 CogVideoX / minimax-video-01 / sora-2" />
        </div>
        <div class="field full">
          <label>提示词 Prompt</label>
      ...
      <button class="btn-gen" :disabled="vidGenerating || !vidForm.prompt || !vidForm.provider_id" @click="generateVideo">
...
export default {
  ...
  data() {
    return {
      ...
      // 图片
      imgForm: {
        provider_id: null, model: 'dall-e-3', prompt: '', n: 1,
  ...
  async mounted() {
    try { this.providers = (await api.getProviders()) || [] } catch (e) { console.error(e) }
  },
  methods: {
    async generateImage() {
