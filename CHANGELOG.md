...
- u1s1 客户端信号（DPoP）：u1s1 平台把赠送额度的 API 推理收紧为「官方客户端专属」——普通 Bearer api_key 一律 403（实测 x-u1s1-* 归因头/设备 token 各种形态都不放行）。现按官方 CLI `device-auth.js` 同协议实现 RFC 9449 DPoP：设备登录时生成并加密持久化 EC P-256 密钥对（`oauth_tokens.device_key_enc`），推理/额度请求逐条现签 `DPoP <u1s1d-…>` + `dpop` proof（需到「OAuth 连接」页重新登录一次 u1s1 以签发设备密钥）；补客户端证明 `x-u1s1-attestation`（第二段握手：GET /v1/models 响应体 `client_attestation.token` 领 7 天 token，官方 CLI 逐推理请求附加；只有 DPoP 时 /v1/models 能 200 但 chat/completions 仍 403「检测到非 u1s1 客户端」，`server/core/u1s1_attestation.py` 对齐 CLI 缓存策略：临期 24h 后台刷、失败冷却 30s、无 token 首拉阻塞 ≤4s）
